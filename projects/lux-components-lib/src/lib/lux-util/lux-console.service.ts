...
export class LuxConsoleService implements OnDestroy {
  ...
  constructor() {
    this.configSubscription = inject(LuxComponentsConfigService).config.subscribe((newConfig: LuxComponentsConfigParameters) => {
  ...
  private static isDebugMode() {
    return LuxConsoleService.config ? LuxConsoleService.config.displayLuxConsoleLogs : false;
  }
