...
import { LuxButtonComponent, LuxIconComponent, LuxInputAcComponent, LuxLinkComponent, LuxPanelComponent, LuxPanelContentComponent, LuxPanelHeaderDescriptionComponent, LuxPanelHeaderTitleComponent, LuxProgressComponent, LuxTabComponent, LuxTabsComponent } from '@ihk-gfi/lux-components';
...
export class LuxChatAiComponent implements OnInit {
  ...
  public set baseUrl(url: string){
    this.luxChatAiService.baseUrl = url;
  }
  ...
  @Input()
  public assistantName = "Mock Chat-AI"

  @Input()
  public standardWelcomeMessage = "Hallo, wie kann ich Ihnen weiterhelfen?";

  @Input()
  public set chatId(chatId: string | undefined) {
    this.loadChat(chatId);
  }

  @Output()
  public chatIdChange: EventEmitter<string> = new EventEmitter<string>();
  ...
  public get chatData(): LuxChatData {
    return this._chatData;
  };

  public set chatData(data: LuxChatData){
    this._chatData = data;
  }
  ...
  public ngOnInit(): void {
    //Check if chat data was already initialized (through input 'chatId')
  ...
  //   this.loadEmptyChat();
  //   //Tell outer component that the chat id is empty => new chat
  //   this.chatIdChange.emit(undefined);
  // }
  ...
  public onHistoryChatClicked(chatSummary?: LuxChatAiSummaryDto){
    this.sidebarHistoryVisible = false;
    this.loadChat(chatSummary?.chat_id);
    this.chatIdChange.emit(chatSummary?.chat_id);
  }
  ...
  private postMessage(query: string, callback?: () => void) {
    ...
    this.luxChatAiService.postChatMessageStream(query, curChatId, this.userName).subscribe({
      next: (streamedData) => {
        if(replyEntry === undefined){
          ...
          console.log("INIT" , streamedData);
          replyEntry = this.chatData.addChatEntry(streamedData.sender_name, "", streamedData.created_at);
      ...
      complete: () => {
        if(replyEntry){
          console.log("Complete!", replyEntry.content);
  ...
  private checkTitleGeneration() {
    if(this.chatData.data.length == 3 || this.chatData.data.length == 7) {
      console.log("len", this.chatData.data.length);
      const id = this.chatData.metadata.chat_id;
      this.luxChatAiService.generateTitleForChat(id).subscribe({
      ...
      this.luxChatAiService.getChatById(chatId).subscribe({
        next: (response) => {
          ...
          for(const entry of response.messages){
            ...
            const loadedMessage = loadedChatData.addChatEntry(entry.sender_name, entry.content, entry.created_at);
