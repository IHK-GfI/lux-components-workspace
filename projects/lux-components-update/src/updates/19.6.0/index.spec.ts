...
describe('update190600', () => {
  ...
  beforeEach(async () => {
    ...
    context = runner.engine.createContext(runner.engine.createSchematic('update-19.6.0', runner.engine.createCollection(collectionPath)));
  ...
  describe('[Rule] update190600', () => {
    it('Sollte die Abhängigkeiten aktualisieren', (done) => {
      ...
      callRule(update190600(testOptions), appTree, context).subscribe(
        () => {
          expect(getPackageJsonDependency(appTree, '@ihk-gfi/lux-components').version).not.toEqual('19.1.0');
          expect(getPackageJsonDependency(appTree, '@ihk-gfi/lux-components').version).toEqual('19.6.0');
          done();
        },
